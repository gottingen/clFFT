...
int main( int argc, char **argv )
{
    ...
    bool s;
    bool m;
    app.add_option( "--verbose,-v",		verbose,	"print out detailed information for the tests" );
    app.add_option( "--noVersion",     noVersion,"Don't print version information from the clFFT library" );
    app.add_option( "--noInfoCL",      noInfoCL,"Don't print information from the OpenCL runtime" );
    app.add_option( "--cpu,-c",         cpu,"Run tests on a CPU device" );
    app.add_option( "--gpu,-g",         gpu,"Run tests on a GPU device" );
    app.add_option( "--all,-a",         all,"Run tests on any device type (default)" );
    app.add_option( "--platform",      g_platform_id,   "Select a specific OpenCL platform id as it is reported by clinfo" )->default_val(0);
    app.add_option( "--device",        g_device_id,   "Select a specific OpenCL device id as it is reported by clinfo" )->default_val(0);
    app.add_option( "--pointwise,-p",         pointwise,"Do a pointwise comparison to determine test correctness (default: use root mean square)" )->default_val(0.01f);
    app.add_option( "--tolerance,-t",        tolerance,   "tolerance level to use when determining test pass/fail" );
    app.add_option( "--numRandom,-r",        number_of_random_tests,   "number of random tests to run" )->default_val(2000);
    app.add_option( "--seed",        random_test_parameter_seed ,
						"seed to use for the random test. defaults to time(NULL)" )->default_val( time(NULL)%1308000000 );
    ...
    app.add_flag( "--short,-s",         s,"Run radix 2 tests; no random testing" );
    app.add_flag( "--medium,-m",         m,"Run all radices; no random testing" );
	...
	size_t mutex = gpu  ? 1 : 0
		| cpu ? 2 : 0
		| all ? 4 : 0;
	if ((mutex & (mutex-1)) != 0) {
		terr << _T("You have selected mutually-exclusive OpenCL device options:") << std::endl;
		if (gpu) terr << _T("    gpu,g   Force selection of OpenCL GPU devices only" ) << std::endl;
		if (cpu) terr << _T("    cpu,c   Force selection of OpenCL CPU devices only" ) << std::endl;
		if (all) terr << _T("    all,a   Force selection of all OpenCL devices (default)" ) << std::endl;
		return 1;
	}
